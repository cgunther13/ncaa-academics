...
kable(summary(mpassengers), digits = 2)
anova(mpassengers)
...
mtotal <- lm(tip_pct ~ fare_amount + I(fare_amount^2) + tolls_amount, 
             data = taxi)
summary(mtotal)
```
...
locationPlot(taxi, taxi$tip_pct, lat_type = "pickup", GRID_RESOLUTION = 30, 
             title = "Percentage Tipped by Time of Day", varname = "TipPct")

locationPlot(taxi, taxi$tip_pct, lat_type = "dropoff", GRID_RESOLUTION = 30, 
             title = "Percentage Tipped by Time of Day", varname = "TipPct")
```
...
m25all <- glm(tip25 ~ extra + fare_amount + passenger_count + tolls_amount + 
              ...
              dropoff_time + factor(pickup_region) + factor(dropoff_region), 
...
In terms of tipping less than 15%, all else equal, a 1 dollar increase in extra
charges is associated with a `exp(coef(m15step)[2])`% increase in the odds of 
tipping less than 15%, and a 1 dollar increase in fare is associated with a 
`exp(coef(m15step)[3])`% increase in the odds of tipping less than 15%. In 
addition, a rider is `exp(coef(m15step)[3])`% more likely to tip less than 15%
than a morning (as well as evening and night rider, since evening and night are 
not significantly different from morning). Finally, riders seem to be 
more likely to tip less than 15% in some regions as well, notably region 15. 
...
Finally, looking at when riders don't tip, all else equal, a 1 dollar increase 
